...
DELIMITER $ $ -- Crea una stored procedure chiamata 'GetCustomers' che non accetta parametri.
CREATE PROCEDURE GetCustomers() BEGIN -- La procedura seleziona nome, città, stato, codice postale e paese dalla tabella 'customers'.
SELECT
    customername,
    city,
    state,
    ...
    country
FROM
    customers -- I risultati sono ordinati per nome del cliente.
ORDER By
    customername;

END $ $ -- Reimposta il delimitatore predefinito a ';'.
DELIMITER;
...
CREATE PROCEDURE GetEmployees() BEGIN -- Seleziona i dati dei dipendenti e li unisce con le informazioni della sede (offices)
...
CREATE PROCEDURE GetOfficeByCountry(IN countryName VARCHAR(255)) BEGIN -- Seleziona tutte le sedi che corrispondono al paese passato come parametro.
SELECT
    *
FROM
    offices
...
CREATE PROCEDURE GetOfficeByCountries(IN countryList VARCHAR(255)) BEGIN -- Costruisce una query SQL dinamicamente come stringa.
-- @query è una variabile di sessione.
SET
    @query = CONCAT(
        'SELECT * FROM offices WHERE country IN (''',
        REPLACE(countryList, ',', ''','''),
        -- Sostituisce le virgole per creare una lista SQL valida.
        ''')'
    );
...
CREATE PROCEDURE GetOrderByStatus(
    ...
    OUT total INT
) BEGIN -- Conta il numero di ordini con lo stato specificato.
SELECT
    COUNT(orderNumber) INTO total -- Assegna il risultato al parametro di output 'total'.
FROM
    orders
WHERE
    status = OrderStatus;
